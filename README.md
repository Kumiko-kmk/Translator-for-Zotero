...
[![CI](https://github.com/Kumiko-kmk/Translator-for-Zotero/actions/workflows/ci.yml/badge.svg)](https://github.com/Kumiko-kmk/Translator-for-Zotero/actions/workflows/ci.yml)
...
[下载安装包](https://github.com/Kumiko-kmk/Translator-for-Zotero/releases/latest) · [开发与交接](docs/DEVELOPMENT.md) · [版本记录](CHANGELOG.md) · [提交问题](https://github.com/Kumiko-kmk/Translator-for-Zotero/issues)
...
- **双 Provider**：支持千问 `qwen-mt-plus` 和 DeepSeek `deepseek-v4-flash`，配置与缓存相互隔离。
- **本地安全存储**：API Key 保存于 Zotero/Firefox Login Manager；成功译文缓存在本地 SQLite。

1.2.0 已移除可靠性不足的“翻译整页”功能。插件只自动翻译标题和摘要；正文由用户主动选择，从而避免公式、表格、图片标签等内容被批量误译。
...
两个 Provider 的密钥、模型和缓存互不混用。切换 Provider 只影响后续请求，不会修改已经显示的译文。
...
```text
Translator-for-Zotero/
├─ plugin/                 # 正式 Zotero 插件源码与运行资源
├─ tests/                  # 当前版本的 Node 与包结构回归测试
├─ tools/build_xpi.ps1     # 唯一受支持的 XPI 构建入口
├─ docs/
│  ├─ DEVELOPMENT.md       # 跨电脑开发、测试和发布交接
...
详细环境准备、文件职责、人工验收和发布流程见 [docs/DEVELOPMENT.md](docs/DEVELOPMENT.md)。最短验证流程：
